...
class StockData:
    def __init__(self,ticker,ratios = True,cash = True, balance = True,est = True,fraud = True,
                    income = True,analysis = True,news = True,sec = True):
        self.ticker = ticker
        self.ratios = ratios
        self.cash = cash
    ...
    def create_file(self,file,file_folder,file_name,file_type):
        file_path = '../ticker/{}/{}/{}'.format(self.ticker,file_folder,file_name + file_type)
        if file_type == '.csv':
    ...
    def news_file(self):
        ...
        if self.news:
            try:
                ...
                result['final_score'] = [max(x,y) for x,y in zip(result[choices[0]],result[choices[1]])]
...
class EconomyData:
    ...
    def treasury_file(self):
        if self.treasury_news:
            try:
                result = openbb.economy.macro(parameters = ['CPI','M3YD','Y10YD','URATE'])[0]
